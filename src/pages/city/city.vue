<template>
    <div class="city_container">
      ...
      <ul class="getpois_ul">
        <li v-for="(item, index) in placelist" @click="goBussPage(index, item.geohash)" :key="index">
          <h4 class="pois_name ellipsis">{{item.name}}</h4>
          <p class="pois_address ellipsis">{{item.address}}</p>
        </li>
      </ul>
      <div v-if="historytitle&&placelist.length" class="clear_all_history" @click="clearAll">清空所有</div>
...
<script>
    ...
    export default {
      ...
      methods:{
        ...
        goBussPage(index,geohash){
          //进入地区商家列表
          let history = getStore('placeHistory');
          let choosePlace = this.placelist[index];
          if(history){
            //如果有历史记录，且当前选择的地址没记录过，则添加入历史记录中
